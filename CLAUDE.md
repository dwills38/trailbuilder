...
| `src/gallery.js` | **Builds Gallery.** Public completed-builds gallery layer. PURE helpers — `gallerySnapshot`/`galleryGate`/`galleryDenorm` build the `{b,p}` publish snapshot, the complete-and-zero-errors publish gate, and the denormalized sort hints straight from the engine — PLUS async Supabase data-access (`publishGalleryBuild`/`listGalleryBuilds`/`getGalleryBuild`/`updateGalleryBuildMeta`/`deleteGalleryBuild`). Loads AFTER `compat.js` (the pure helpers read the engine globals in the browser; `require('./compat.js')` in Node tests). Browser glue — excluded from typecheck like account/forum. |
...
| `supabase/gallery-builds.sql` | **Builds Gallery migration.** `gallery_builds` table (snapshot-on-publish, NOT a flag on `builds` — the private garage stays untouched) with public-read RLS, author-or-admin write (reuses `is_forum_admin()`), per-user publish caps (10/day, 200 total), and CHECK caps. Run AFTER `schema.sql` + `forum-profiles.sql`. Full adversarial-RLS walkthrough in the header. |
...
| `test/test-gallery.js` | Builds Gallery publish logic (`src/gallery.js`, pure, no network): the published payload round-trips through `sanitizeShare()`, the publish gate = complete + zero errors (INCL. a DJ brakeless single-speed build publishing cleanly), denormalized totals equal `buildTotals()`, and `esc()` neutralizes HTML in the public title/story. |
...
| `tools/gen-sitemap.js` | **Builds Gallery sitemap generator** (zero deps, Node 18+ global fetch). Emits `sitemap.xml` — the homepage + one `?g=<id>` URL per published gallery build — reading the public Supabase URL/anon key straight out of `src/config.js`. Fail-soft (homepage-only on any error). Run by `deploy.yml` (which also has a daily `schedule` trigger to keep the sitemap fresh). |
...
| `.github/workflows/deploy.yml` | GitHub Actions — deploys the static app (`index.html` + `src/`) to GitHub Pages on push to `main`, after `validate` + `tests` pass. Also regenerates the Builds-Gallery `sitemap.xml` (via `tools/gen-sitemap.js`) into the Pages artifact, and carries a daily `schedule` trigger so the sitemap refreshes even with no code push. Needs a public remote + Pages source = "GitHub Actions". |
