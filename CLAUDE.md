...
| File | Purpose |
|------|---------|
| `index.html` | The app (UI). Plain HTML/CSS/JS, no build step. Loads `data/mtb.js` then `src/compat.js` via `<script>` (**that order is load-bearing** — see below). Open by double-click. Never loads the BMX engine/data — `bmx.html` is a separate page for that. |
| `src/compat.js` | The MTB compatibility **engine** (`checkBuild`) + helpers (`compatOf`, `buildTotals`, `GROUPS`/`SLOTS`, `ALIASES`/`canonicalId`). The heart of the project. Reads the catalog from `data/mtb.js` and re-exports it as `PARTS`, so every consumer (`validate.js`, `test/**`, `tools/**`, the browser modules) is unchanged by the split. |
| `data/mtb.js` | **The MTB catalog data** (`MTB_PARTS_RAW` — rows *and* presets), split out of `compat.js` on 2026-07-23 so the engine is readable/greppable on its own (compat.js went 22,942 → ~2,100 lines). MTB was the last catalog fusing data and engine; this matches `data/bmx.js`+`compat-bmx.js`, `data/road.js`+`compat-road.js`, etc. **`index.html` must `<script>` it BEFORE `src/compat.js`** (compat.js binds `PARTS` at load time — unlike `bmx.html`, whose engine holds no catalog and can load either order), and **`deploy.yml` must stage it explicitly** (`cp data/mtb.js _site/data/`) or the flagship page 404s in production. `test/test-mtb-data-split.js` guards both. |
...
| `src/page-shell.js` | **Shared page chrome** (2026-07-23, efficiency finding 2.3): the ONE definition of the two chrome blocks that had actually drifted — the footer **legal-link tail** (`TB_LEGAL_LINKS` + `tbLegalLinksHtml`; five of seven pages were found missing it on 2026-07-23) and the header **family switcher's surface list** (`TB_FAMILY` + `tbFamilyMenuItemsHtml`; every go-live previously meant six hand-edits). Loaded by a classic `<script src>` right after `src/ui-common.js` on the six builder pages; mounts into `<span data-tb-legal>` / `<span data-tb-family="<id>">` placeholders (the family one via `outerHTML`, so the anchors stay direct children of `.fam-pop` for the CSS). Derives its base path prefix from its own `<script src>`, so KitBuilder's `../` needs no per-page config. **Deliberately narrow:** the review's "82% identical" is a road↔gravel PAIRWISE maximum — across all six builders only ~9% of the `<style>` blocks and ~24% of the `<header>`s match, and the theme token blocks (`:root`/`html.dark`/`html.loam`/`html.rad`) have 3–5 genuinely different variants each, so they stay per-page. The five STATIC content pages (home/about/privacy/terms/affiliate-disclosure) keep literal legal links on purpose — they must read with JS off — and `test/test-page-shell.js` checks them against the same canonical list. Reads the DOM through `globalThis` (the `compat-bmx.js` idiom) so it stays typechecked. |
...
| `src/service-log.js` | **Garage service log** (Feature slate #2, 2026-07-21): the rider's own maintenance notebook — pure logic behind the click-opened 🔧 Service log dialog. `normalizeServiceEvent()` validates a form (non-empty "what was done", real past-or-today date, a build/part target) into the insert payload; `serviceLogModel()` resolves fetched rows' targets (build names via the garage list, part ids through `canonicalId`→`byId`→`nameOf`, a deleted build's rows kept with an honest label); `renderServiceLogHtml()` escapes everything via injected `esc()`. No schedule/interval claims — it records only what the rider wrote. Deps injected like `build-sheet.js`; zero engine changes. Data access in `account.js` (`hasServiceLog` feature-detect + list/add/delete); storage = the owner-only-RLS `service_events` table (schema.sql Phase 5, SETUP.md §10). |
| `src/build-diff.js` | **Build diff** (Feature slate, 2026-07-21): pure model + renderer behind the click-opened ⇄ Build diff dialog (garage footer, signed-in only) — `buildDiffModel()` folds two resolved builds (A = baseline → B) into per-slot changes classified added/removed/swapped, `buildTotals` passthrough for both sides + signed price/weight deltas (weight delta null against a weightless side, ± hedge on sample gaps), and the verdict delta: `checkBuild` on both sides diffed BY `verdictKey` (the dots' identity contract) into only-in-A/only-in-B lists with the engine's messages VERBATIM plus a disclosed shared count. `renderBuildDiffHtml()` escapes everything via injected `esc()`; "only in" set-membership wording, never a causality or all-clear claim. Deps injected like `build-sheet.js`; zero engine changes. |
| `src/upgrade-optimizer.js` | **Upgrade Optimizer** (Feature slate, 2026-07-21): pure model + renderer behind the click-opened ⚖️ Upgrade optimizer dialog (build panel, shown once any part is picked) — `upgradeOptimizerModel()` ranks every same-category catalog swap for each filled slot by COST PER GRAM SAVED (candidate MSRP ÷ grams lighter), HONEST MATH ONLY: a row exists only when BOTH weights are ✓ Verified (the verify bar checks weight); sample-weight candidates land in a disclosed exclusion ledger, a sample-weight CURRENT part makes its slot a named not-rankable note — never a silent guess. `streetPrice` never feeds the ranking (pricing policy); discontinued/recalled excluded (not buyable new at MSRP); compat-gated through upgrade.js's injected `upgradePlacementDiff` (new error → excluded; warning-only → ranked yellow with the engine message verbatim). Ordering is pure brand-blind math (ties: bigger saving, then id). `renderUpgradeOptimizerHtml()` escapes via injected `esc()`, disclosed display cap, the unbiased-math + MSRP sentences, the app's sample-data disclosure verbatim. Deps injected like `build-diff.js`; zero engine changes. |
| `src/discontinued-archive.js` | **Discontinued archive** (Feature slate, 2026-07-21): pure model + renderer behind the click-opened 🗄 Discontinued archive dialog (⋯ menu) — `discontinuedArchiveModel()` folds injected catalog source(s) `{label, parts}` into per-source, per-category rows of every `status:'discontinued'` part (recalled/current excluded — recalls have `recall-notes.js`), resolving `supersededBy` strictly WITHIN its own source (a chained successor carries its own lifecycle status; an unresolvable pointer stays raw, disclosed as such); `renderDiscontinuedArchiveHtml()` escapes everything via injected `esc()`, says "No recorded successor" (our data, never nonexistence). Catalog-agnostic on purpose: each page wires only the catalog(s) it already loads — index.html passes `PARTS` only (it must never load BMX/e-MTB data, hard rules 1 & 3). Deps injected like `build-sheet.js`; zero engine changes. |
| `src/owned-vs-buy.js` | **Owned-vs-to-buy planner** (Feature slate, final pick, 2026-07-21): pure model + renderer behind the click-opened 🧰 Owned vs to-buy dialog (build panel, signed-in only — inventory is account data) — `ownedVsBuyModel()` folds the current build + the rider's inventory into a per-slot owned/to-buy split: ownership by EXACT catalog id through `canonicalId` (retired inventory ids match their renamed part; unresolvable ids counted, disclosed), QUANTITY-HONEST allocation (one owned unit covers one slot, SLOTS order; the overflow slot is to-buy with the reason spelled out), owned-alternative suggestions for to-buy slots compat-gated through upgrade.js's injected `upgradePlacementDiff` (new error → excluded + counted; warning-only → suggested 'w' with the engine messages verbatim), and the honest remaining-cost subtotal: each missing part's own MSRP (`streetPrice` never feeds it; kit/bundle pricing NOT applied — disclosed whenever the build total bills a bundle). `renderOwnedVsBuyHtml()` escapes via injected `esc()`; matched-by-part-not-condition wording, the hedged no-new-conflicts sentence, the app's sample-data disclosure verbatim — never a compatibility guarantee. Deps injected like `build-diff.js`; zero engine changes. Data access (`listInventory`) stays in `account.js`. |
...
| `test/test-service-log.js` | The garage service log (`src/service-log.js`): the normalizer's honesty gates (future dates, empty titles, missing target, real-calendar dates), target resolution incl. retired ids + deleted-build survival, date-desc sorting/filtering, and renderer escaping with the no-schedule-claims pin. |
| `test/test-build-diff.js` | The build diff (`src/build-diff.js`): added/removed/swapped classification by slot + part id, totals passthrough with derived deltas (incl. per-side presetBy and the null-not-fabricated weight delta), verdictKey-identity verdict diffing with verbatim messages, SLOTS ordering, and renderer escaping + the honest-framing pins (hedged no-difference sentences, the app's sample-data disclosure verbatim, never "All compatible"). |
| `test/test-upgrade-optimizer.js` | The upgrade optimizer (`src/upgrade-optimizer.js`): exact $/g arithmetic on MSRP (streetPrice pinned out), the honesty gates both directions (sample-weight candidate → counted exclusion; sample-weight current part → named slot note, lighter verified candidates still refused), the compat gate (error-adding swap excluded; warning-only ranked 'w' with the engine message verbatim), the not-lighter/not-buyable/no-price ledger, brand-blind deterministic tie-breaks, per-position tire judging, purity + determinism, a real-catalog invariant sweep over the golden Megatower build, and renderer escaping + the honest-framing pins ("never silently ranked", the unbiased-math sentence, disclosed display cap, the app's sample-data disclosure verbatim, never "All compatible"). |
| `test/test-discontinued-archive.js` | The discontinued archive (`src/discontinued-archive.js`): discontinued-only inclusion (recalled/current excluded), same-source-only `supersededBy` resolution incl. chained/recalled successors + raw dangling pointers, catalog-order grouping and name/year sorting, a real-catalog sweep (every discontinued `PARTS` row present, every pointer resolves), and renderer escaping + the honest-framing pins ("No recorded successor", the dangling-pointer disclosure, the app's sample-data disclosure verbatim). |
| `test/test-owned-vs-buy.js` | The owned-vs-to-buy planner (`src/owned-vs-buy.js`): exact-id ownership incl. retired-alias resolution and alias+canonical qty merging, the quantity-honest allocation (qty 1 across two same-part slots → one owned + one disclosed to-buy), the compat gate on owned alternatives (error-adding swap excluded + counted; warning-only suggested with the engine message verbatim; clean 'g' first), unknown-inventory disclosure, the MSRP-only subtotals + the kit-gap flag, purity/determinism, and renderer escaping + the honest-framing pins (matched-by-part wording, hedged suggestions, the app's sample-data disclosure verbatim, never a guarantee). |
| `test/test-page-shell.js` | The shared page chrome (`src/page-shell.js`): the pure renderers (legal tail incl. the `../` base, family list incl. `here`/`aria-current` marking and About-note escaping), **plus a CROSS-PAGE guard over all 11 served pages** — every page carries the analytics beacon and all three legal links, no builder page hand-writes either block again, the shell loads after `ui-common.js`, the static pages spell the legal links identically to the canonical list, every `TB_FAMILY` surface is a page that exists **and is staged by `deploy.yml`** (an unstaged page in the menu is a production 404), and the shell opens/appends/schedules nothing (hard rule 2). This is the file that would have caught the 2026-07-23 missing-legal-links and missing-beacon drift. |
...
- **shifter / derailleur / cassette / chain**: `system` (`sram-eagle`/`sram-transmission`/`shimano-12`/`shimano-11`/`sram-11` — the 11-speed tiers went live 2026-07-07/10; two cb workers have skipped real builds off this line when it was stale, keep it current), `speeds`; shifter + derailleur also `actuation` (`cable`/`electronic` — same system, but a trigger can't drive an AXS mech); shifter optional `clampType` (`ispec-ev`/`ispec-ii`/`ispec-b`/`matchmaker`/`band`/`pod`); derailleur also `maxCog`,`mount`; cassette also `freehub`,`minCog`,`maxCog` (numeric — the display range is derived; HG floor is 11T, validator-enforced).
...
"fits"), and an **I.S. caliper** on any other mount stays an ERROR — **still DORMANT: no I.S.-BODY caliper is
cataloged.** (A 2026-07-23 row, `bk-bengal-helix11-is`, was briefly entered as `mount:'IS'` and was
corrected to `'PM'` the same day by `audit-wave-2026-07-23` finding H1: its source sentence had been
quoted only as far as the I.S. clause, but continues *"Front brake also compatible with Manitou fork
Post-mount (74mm) by dismounting the adaptor"* — the body is Post-mount with an I.S. adaptor fitted.
As `'IS'` it hard-errored against 350 of 365 PM frames and every fork, a false "won't fit".) The
PM-caliper-on-I.S.-frame adapter WARNING is and was already live on the **12** I.S. frames (Cotic
BFe/Jeht, Banshee Paradox V3, Commencal Meta HT V2, both Surlys, both Chromags, Production Privee
Shan GT, Dartmoor, Identiti, Canfield); the I.S. *fork* branch is dormant too);
