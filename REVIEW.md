...
| 15 | Minor | ~~Rules 2-front/8/11 are dead code — single-valued vocabularies mean they can never fire, and none has a test~~ **Largely resolved:** the vocabs widened with real DH parts. Rule 8-front and rule 11 now fire on real-part builds (with tests, incl. real-part error cases); rule 2-front has real live vocab but its *mismatch/error* case is still only pinned synthetically. | rules 2/8/11 + VOCAB |
...
| 2. Axles | Both halves live | Front-axle vocab widened — real DH forks (`20x110`, `20x110-nonboost`) + `20x110` front hubs now build silently as matched pairs. The *mismatch/error* case is still only exercised by a synthetic test override (#15). |
...
| 8. Brake mount | Live (front half) | FM (flat-mount) calipers now in the catalog (e.g. `bk-shimano-xtr-m9110-fm`); `front-brake-mount` fires on a real FM-caliper-on-PM-fork build (`test-verdict-audit`). PM *size* still unmodeled — delegated to rule 10, which lacks a minimum (#3, #15). |
...
| 11. Steerer | Live | Real dual-crown (`straight-dc`) forks + DH frames now exist; both the matched and the mismatched cases are covered, including a real-part error test (`test-verdict-audit`) (#15). |
...
- **#15 — single-valued vocabularies make rules 2-front/8/11 dead code** — **Minor; largely RESOLVED.** *Original finding:* all parts were PM/tapered/Boost110, so no constructible build could fire the front-axle, brake-mount, or steerer checks, and greps showed zero test coverage. *Update (2026-07):* the catalog's DH additions widened all three vocabs, and the checks now fire on **real** parts:
  - **Rule 8-front (brake mount): fully live.** Flat-mount (`FM`) calipers are catalog parts (`bk-shimano-xtr-m9110-fm` and siblings); `front-brake-mount` raises on a real FM-caliper-on-PM-fork build, pinned by a real-part error test in `test-verdict-audit.js`.
  - **Rule 11 (steerer): fully live.** Real dual-crown `straight-dc` forks (Fox 40, the BoXXers, the Öhlins DH38s) and `straight-dc` DH frames (Commencal Supreme DH V5, Transition TR11, Santa Cruz V10) exist. Both the matched (silent) and the mismatched (error) cases are covered, including a **real-part error test** (`test-verdict-audit.js`: real tapered ZEB on the real straight-dc Supreme DH → `steerer` error) alongside the synthetic pins in `test-engine.js`.
  - **Rule 2-front (front axle): live vocab, error case still synthetic-only.** Real DH forks (`20x110`, `20x110-nonboost`) and `20x110` front hubs now build silently as matched pairs, so the rule is no longer dead code. But its *mismatch/error* branch is still exercised only by a synthetic test override (`test-engine.js` "front-axle check fires on a synthetic non-Boost fork") — there is no real-part error-case test.

  The remaining latent risk is confined to rule 2-front. **Open question (for the coordinator + Douglas):** do we want a real-part error-case test for rule 2 (and to belt-and-brace rule 11)? A dual-crown DH fork on a trail frame — the shape a real-part axle mismatch would take — is a nonsensical build, and a wrong or weak test is worse than none; deferred here rather than authored. **Note:** "18 rule areas" wording is itself stale — the engine is now described as 20 rule areas.
