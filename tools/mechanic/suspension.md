...
**SUS-50 — RockShox Bottomless Token part-number selection is keyed to MODEL CODE and SPRING
...
**SUS-51 — RockShox rear-shock volume-reduction hardware CHANGED ARCHITECTURE across generations:
...
sourced instance of the generational-trap pattern. *Confidence: confirmed.* Source: as SUS-50
(p.2). Cross-reference: SUS-36 (Deluxe token maxima), SUS-40.

**SUS-52 [CLOSES the named "no numeric coil-spring-rate selection chart" gap, at the tier the
...
**SUS-53 [premise correction — for the Gaps list, not the engine] — the "frame-leverage-adjusted
...
SUS-52's fork chart. Those are two different problems: a **fork's** coil spring acts essentially
directly on the axle path (no linkage multiplication), so a fork spring chart is correctly keyed
to rider weight alone — which is exactly what SUS-52 shows RockShox publishing. **Leverage ratio
...
restructure:** treat "fork coil spring selection" (CLOSED, SUS-52) and "rear coil spring rate vs
...
SUS-52, and the chapter's open leverage-ratio/kinematics gap.

*Coordinator note (2026-07-18, seat 13): these four facts were authored as SUS-48–51 but
renumbered to SUS-50–53 on merge — SUS-48/49 were already taken on main by master-1's
cherry-picked coil-service facts above. Content unchanged; internal cross-references were
renumbered with them. Ids stay append-only.*
