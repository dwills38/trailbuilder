...
var EMTB_PARTS = [
  ...
  },
  {
    ...
    desc: 'Verified vs the fetched maker page (canyon.com Spectral:ON CF 8). Model/wheel/battery/travel/drivetrain/brakes/weight/price corrected verify/emtb-4 wave 4 cluster A: the generic "Spectral:ON" row actually described no real single SKU — the platform is mullet (29in front/27.5in rear), not 29in-only; battery is 800Wh removable, not 900Wh fixed; rear travel is 155mm, not 150mm; and no current mainline trim runs SRAM GX Eagle Transmission/SRAM Code (that combo matched only the CF 9 AXS trim, now in Canyon\'s outlet/clearance section, not the active lineup) — retitled to the real, currently-sold CF 8 SKU with its confirmed Shimano XT/SLX drivetrain and SLX brakes. Weight (24.44kg) and price ($5,999 MSRP, site currently shows a $4,499 sale) both maker-page-sourced. PRICE-BASIS-LOCALE-1 (2026-07-24): the stored source was the /en-gb/ page, which this audit flagged as GBP-priced (re-fetched: it currently shows £3,819, not USD, contradicting the "$5,999 MSRP...maker-page-sourced" claim above — that claim was reading a UK page as if it were USD). RE-FETCHED canyon.com/en-us (same product path) directly: the Spectral:ON CF 8 outlet/clearance listing shows this exact SKU at a genuine $5,999 USD original/MSRP price (struck through, with $4,199-$4,499 sale variants by colorway) — a real US MSRP confirming the stored price exactly. Source corrected to the /en-us/ page so msrp-confirmed is genuinely US-page-backed; price/priceBasis unchanged (already correct).',
    verified: true, priceBasis: 'msrp-confirmed', lastChecked: '2026-07-24', source: 'https://www.canyon.com/en-us/electric-bikes/electric-mountain-bikes/spectral-on/spectral-on-cf/spectral-on-cf-8/4077.html'
  },
  {
  ...
  },
  {
    ...
    desc: 'Verified vs Santa Cruz product page. Model/trim corrected verify/emtb-4 wave 4 cluster D (generic "Heckler SL" -> real current SKU "Heckler SL GX AXS"); travel corrected 150/140 -> maker-published 160/150 (matches the full-power Heckler); weight corrected 19.1kg sample -> maker-published 19.88kg (43.84lb). Price kept as pre-existing same-tier sample — Loam Wolf review cites a 5-model SL range of EUR 7,499-12,999, no confirmed US MSRP for this exact build. PRICEBASIS BACKFILL (pb-emtb-clear, 2026-07-22): re-fetched the stored source URL (browser pane) — the page now geo-serves USD pricing (URL still /en-eu/, unchanged) and discloses an exact literal price, "$9,699.00 USD". Price corrected 7500 (stale EUR-range-derived sample) -> 9699 confirmed USD MSRP; msrp-confirmed set; source URL unchanged. PRICE-BASIS-LOCALE-1 (2026-07-24): flagged for the stored /en-eu/ source URL. RE-FETCHED it directly this session and it is now geo-defaulting to EUR (€9,799 regular/€6,299 sale) for this session\'s IP — the /en-eu/ path is genuinely locale-dependent, confirming the flag was righteous even though the 2026-07-22 fetch happened to see USD. Found the real fix: santacruzbicycles.com/products/heckler-sl-gx-axs-2026-1-1 (no locale segment at all) is the canonical US-serving URL and RE-CONFIRMED it live at exactly "$9,699.00 USD", matching the stored price exactly with identical specs (19.88kg, 160/150mm travel). Source corrected to the locale-free canonical URL so msrp-confirmed no longer depends on session geo-luck; price/priceBasis unchanged (already correct).',
    verified: true, priceBasis: 'msrp-confirmed', lastChecked: '2026-07-24', source: 'https://www.santacruzbicycles.com/products/heckler-sl-gx-axs-2026-1-1'
  },
  {
  ...
  // ---------------------------------------------------------------------------
  {
    ...
    desc: 'Verified vs the fetched maker page (canyon.com Spectral:ON CFR). Wheel/battery/travel/drivetrain/brakes/price corrected verify/emtb-4 wave 4 cluster A: mullet not 29in-only, 800Wh removable not 900Wh fixed, 155mm rear travel not 150mm, SRAM X0 Eagle AXS Transmission not XX SL, SRAM Maven Ultimate not Maven Silver. Weight (23.9kg) already matched the sample exactly. Price corrected to the $9,499 US MSRP (base-page listing shows a current $6,899 sale); the row already used the CFR name, which correctly identifies this real current top-tier SKU. PRICE-BASIS-LOCALE-1 (2026-07-24): the stored source was the /en-gb/ page (non-US locale), which the prior "$9,499 US MSRP" claim above did not actually come from — RE-FETCHED canyon.com/en-us (same product path) directly: the outlet/clearance listing confirms this exact SKU\'s original/crossed-out price is $9,499 USD (current sale $6,599) — a real US MSRP matching the stored price exactly. Source corrected to the /en-us/ page so msrp-confirmed is genuinely US-page-backed; price/priceBasis unchanged (already correct).',
    verified: true, priceBasis: 'msrp-confirmed', lastChecked: '2026-07-24', source: 'https://www.canyon.com/en-us/electric-bikes/electric-mountain-bikes/spectral-on/spectral-on-cfr/spectral-on-cfr/3530.html'
  },
  {
  ...
  },
  {
    ...
    desc: 'Verified vs the fetched maker page (canyon.com Neuron:ON AL 9) plus Canyon\'s own 2025 launch press release. Motor brand corrected verify/emtb-4 wave 4 cluster A (Shimano EP8 -> Bosch Performance Line CX — another wrong-motor-maker catch) plus battery/travel/drivetrain/brakes (630Wh -> 800Wh, 140/130mm -> 150/145mm travel, Shimano XT -> SRAM S1000 Eagle AXS Transmission, Shimano XT 4-piston -> SRAM DB8 Stealth 4-piston) and model retitled Neuron:ON -> Neuron:ON AL 9 to name the real top-tier SKU actually matching the drivetrain/brake spec. Weight (24.9kg) is maker-page-listed. Price: no US canyon.com listing found; EU RRP is EUR 5,499 (GBP 5,149) — used a US retailer (thebicyclesshops.com) selling the same SKU at $6,439 as the USD sample basis, stated here per THE PRICE RULE. PRICE-BASIS-LOCALE-1 (2026-07-24): this row was flagged for citing a non-US (.co.uk-pattern /en-gb/) source under priceBasis:msrp-confirmed, and this desc\'s own prior text already discloses the real basis: no US canyon.com listing exists at all (RE-CONFIRMED this session: canyon.com/en-us does not list any Neuron:ON model in its current lineup), and the £5,149/€5,499 stored price is not even a straight currency conversion — it\'s a THIRD-PARTY US RETAILER\'S listing ($6,439 via thebicyclesshops.com), which is exactly what priceBasis:third-party-listed is for (distinct from regional-conversion, which is a maker price merely in another currency). msrp-confirmed was a plain mislabel; relabeled to third-party-listed. Price/source unchanged (the $6,439/canyon.com-en-gb pairing is honestly disclosed above, just filed under the wrong basis token).',
    verified: true, priceBasis: 'third-party-listed', lastChecked: '2026-07-24', source: 'https://www.canyon.com/en-gb/electric-bikes/electric-mountain-bikes/neuron-on/al/neuron-on-al-9/4059.html'
  },
  {
  ...
  // ---------------------------------------------------------------------------
  {
    ...
    disciplines: ['e-trail', 'e-enduro'], weight: 23600, price: 5920.12,
    note: "Haibike's flagship full-power all-mountain bike on the Yamaha PW-X3 motor (250W/85Nm/720Wh InTube battery).",
    desc: 'Corrected verify/emtb-4 wave 4 cluster B. Model/trim corrected: no Haibike AllMtn trim is numbered "9" — the current lineup runs AllMtn 1/2/3, and 3 is the top full-power trim (Shimano Deore drivetrain + Deore BR-MT420 4-piston brakes, RockShox 35 Gold RL fork, RockShox Deluxe Select R shock, 160mm front AND rear travel). Frame corrected carbon -> aluminum, rear travel 150 -> 160mm, drivetrain SRAM GX Eagle Transmission -> Shimano Deore (real spec, no SRAM Transmission on any current AllMtn trim), brakes SRAM Code -> Shimano Deore 4-piston. Previously flagged verify/emtb-3 as "PW-X3/85Nm/720Wh matches current AllMtn pages exactly" — that motor/battery reading holds; the frame/drivetrain/brake fields did not and were corrected here. verify/emtb-brands-2 (2026-07-23): re-fetched haibike.com/en-int/products/allmtn-3-hmbr1 directly — confirms "Motor: Yamaha PW-X3, 250W, 85Nm, 25km/h" and "Battery: Yamaha InTube 720Wh" exactly, plus the maker\'s own EUR 5,199 price (no complete-bike weight stated on the page; the existing 23.6kg stays a sample). PRICE: EUR 5,199 maker-confirmed converted at the 2026-07-23 open.er-api.com EUR->USD rate (1.138703) -> $5,920.12.',
    verified: true, priceBasis: 'regional-conversion', lastChecked: '2026-07-23', source: 'https://haibike.com/en-int/products/allmtn-3-hmbr1'
  },
  {
...

  {
    ...
    batteryWh: 630, batteryRemovable: true, assist: 'full-power', display: 'top-tube',
    wheelConfig: '29', suspension: 'full', travelFront: 170, travelRear: 160,
    frameMaterial: 'carbon', drivetrain: 'SRAM X0 Eagle Transmission 12-speed', brakes: 'SRAM Code RSC 4-piston',
    disciplines: ['e-enduro'], weight: 24100, price: 10999, modelYear: 2026,
...

  {
    ...
    desc: 'Fetched directly from the maker\'s own product page (simplon.com/en/Bikes/E-Mountain-Bikes/Rapcon-e_b_1149826), re-confirmed verify/emtb-verify-1: "Frame material Carbon", "Travel (rear/front) 160 mm / 160 mm", "Motor BOSCH PL CX Gen5", "Battery 600 / 800 Wh", "Total bike weight from 22,8 kg", "fork travel/length/OFFSET 160/576/44 mm", "shock size (eye to eye length X HUB) 205 x 62,5 mm", "lower shock hardware Trunnion", starting price "from EUR 6,999.00". Cataloged at the larger 800Wh battery option (the page offers both 600/800Wh — no single fixed-spec SKU is pinned, so this row represents the top-line CORE/Ultimate-package spec); brakes not stated on the fetched page section, left uncataloged. Both interfaces and the maker-published "from 22,8 kg" complete-bike weight are page-confirmed, meeting the verification bar (weight and price both carry an honestly-disclosed "from"/range caveat — no single fixed SKU is pinned, stated here rather than hidden). PRICE: EUR 6,999.00 ("from") maker-published starting price converted at the 2026-07-23 open.er-api.com EUR->USD rate (1.141079) -> $7,986.61; Simplon has no US retail presence, no USD MSRP exists — disclosed regional-conversion "from" sample, not a pinned single-SKU US MSRP. catalog/emtb-breadth-brands-1 wave (first Simplon row — brand was fully absent); promoted to verified:true verify/emtb-verify-1.',
    verified: true, priceBasis: 'regional-conversion', lastChecked: '2026-07-23', source: 'https://www.simplon.com/en/Bikes/E-Mountain-Bikes/Rapcon-e_b_1149826'
  },

  {
    ...
    desc: 'Fetched directly from the maker\'s US shop page (thomus.com/products/lightrider-e2-pro): "Regular price $8,899.00" (SHIMANO SLX build) / "$9,999.00" (SHIMANO XT build), modular frame supporting "140 to 170 millimeters" of travel, "FOX or DT-Swiss" suspension options (stock builds feature Fox Factory 160mm fork per the page text) — cataloged at 160/160mm as the page\'s named stock spec. The US product page does not state the motor/battery for the current build. catalog/emtb-breadth-brands-1 wave (first Thömus row). verify/emtb-brands-2 (2026-07-23): fetched the maker\'s own EU platform page (thoemus.ch/slikerproultimate/ybslikerx/new-lightrider-e2/) for this exact Lightrider E2 platform, which states "Der neue Shimano EP8 Motor" (the current-generation motor replacing the older Shimano E8000/EP6) at "85Nm" and "726Wh" battery — CORRECTED motorModel EP6 -> EP8 (the platform\'s real current motor; EP6 was never actually documented for this SKU, just carried forward from an older review). Battery/travel/price otherwise unchanged, price already confirmed on the fetched US shop page ($8,899, msrp-confirmed-quality but priceBasis withheld here since this pass\'s primary fetch was the EU spec page, not the US price page).',
    verified: true, lastChecked: '2026-07-23', source: 'https://thoemus.ch/slikerproultimate/ybslikerx/new-lightrider-e2/'
  },

  {
    ...
    frameMaterial: 'carbon', drivetrain: 'Shimano XTR Di2 1x12', brakes: 'Shimano XTR (200/180mm)',
    disciplines: ['e-trail'], weight: 17010, price: 14176.85, modelYear: 2026,
    note: 'Thömus\' 2026 lightweight-class flagship — pairs the Swiss-made maxon BIKEDRIVE AIR S motor (a full-power-torque motor in a lightweight-class 2.0kg package) with a compact 400Wh battery; won E-MOUNTAINBIKE Magazine\'s 2026 Editors\' Choice. Deliberately purist minimal-weight setup rather than the maker\'s big-battery E2 platform. This row is the tested "TRAIL-Light" build (140mm front / 130mm rear).',
    desc: 'Sourced from E-MOUNTAINBIKE Magazine\'s fetched first-ride test (ebike-mtb.com/en/thoemus-lightrider-e-max-2026-test/ and the companion Editors\' Choice piece): "maxon BIKEDRIVE AIR S motor... 400 Wh battery... 90 Nm of torque and peak output of 620 watts", "17.01 kg in size M", "EUR 12,450", "140 mm up front and 130 mm at the rear" (the tested TRAIL-Light build), Shimano XTR Di2 1x12 drivetrain, Shimano XTR 200/180mm brakes. verify/emtb-brands-2 (2026-07-23): fetched the maker\'s own thoemus.ch/en.thoemus.ch platform page directly — confirms "maxon Air S motor... 620 watts of peak power at 90 Nm" and "Available as a trail or cross-country version" (matching this row\'s TRAIL-Light build), starting price "from CHF 5,990" (base model; this row stays priced at the tested TRAIL-Light build\'s EUR 12,450 rather than the cheaper base price, since the weight/spec are the TRAIL-Light\'s). CORRECTED travelRear 140 -> 130 (the real TRAIL-Light spec, previously miscataloged as symmetric 140/140); added the confirmed drivetrain/brakes. PRICE: EUR 12,450 converted at the 2026-07-23 open.er-api.com EUR->USD rate (1.138703) -> $14,176.85.',
    verified: true, priceBasis: 'regional-conversion', lastChecked: '2026-07-23', source: 'https://en.thoemus.ch/bikes/lightrider-e-max/'
  },

  {
    ...
    disciplines: ['e-trail', 'e-enduro'], weight: 22800, price: 14741.68, modelYear: 2026,
    note: 'Finnish maker Pole Bicycles\' 2026 comeback e-MTB (founder Leo Kokkonen\'s return after the original Pole Voima/bankruptcy) — CNC-machined 7075-T6 aluminum frame with adjustable-travel geometry (152-200mm rear / 160-200mm front via a shock-mount swap, one frame platform, 12 configurations), full 29er or mullet wheel configuration, built to order (~50 frames/year in Finland). Cataloged at the gravity-spec 170/170mm build; price is the maker\'s own "starting from" complete-build figure, not a fixed price for this exact configuration (fully bespoke per-order).',
    desc: 'Sourced from E-MOUNTAINBIKE Magazine\'s fetched hands-on preview (ebike-mtb.com/en/new-pole-hiisi/), which quotes Pole founder Leo Kokkonen directly: "maxon Air S mit 90 Nm | 600 Wh | 22.8 kg in gravity build specification (manufacturer claim)". Additional detail: "CNC-machined aluminium frame... 160-200mm front travel / 152-200mm rear travel... Choice of 600 Wh or 400 Wh battery (both permanently integrated)... a 250 Wh range extender... maxon AIR S... 2.03 kg, 90 Nm of torque and a peak output of 620 watts". No fixed build kit/drivetrain spec found (the brand sells built-to-order); drivetrain/brakes left uncataloged rather than guessed. verify/emtb-brands-2 (2026-07-23): fetched polebicycles.com\'s own pages directly (polebicycles.com/pages/hiisi + polebicycles.com/pages/faq) — confirms "The Maxon Air S amplifies what you do" motor and "600 Wh internal battery, 400 Wh option, 250 Wh range extender available", matching this row exactly; the FAQ page also states the maker\'s own complete-build starting price: "Starting from €9,084 for the frameset and €12,946 for a complete build" (VAT-exclusive) — a real maker-quoted figure, replacing the prior editorial-sourced "from EUR 15,239" estimate. This is a bespoke starting price, not this exact 170/170mm gravity-spec build\'s confirmed total. PRICE: EUR 12,946 (complete-build starting price) converted at the 2026-07-23 open.er-api.com EUR->USD rate (1.138703) -> $14,741.68.',
    verified: true, priceBasis: 'regional-conversion', lastChecked: '2026-07-23', source: 'https://polebicycles.com/pages/faq'
  },
...

  {
    ...
    motorBrand: 'bosch', motorModel: 'Bosch Performance Line CX Smart System', motorTorque: 85, motorPowerPeak: 600,
    batteryWh: 750, batteryRemovable: true, rangeExtender: true, assist: 'full-power',
    wheelConfig: 'mullet', suspension: 'full', travelFront: 180, travelRear: 160,
    frameMaterial: 'aluminum', drivetrain: null, brakes: null,
    disciplines: ['e-enduro'], weight: 26700, price: 7969.78, modelYear: 2026,
    ...
    desc: 'Fetched directly from the maker\'s own product pages (focus-bikes.com/int/bikes/e-mountainbike/focus-sam2 overview + focus-bikes.com/es_en SAM² 6.8 SKU page): "With 180/170 mm travel" (frame/fork) — note the fork-travel figure on the overview differs slightly by market page (170mm vs 180mm across regional listings); this SKU\'s own page states "180 mm" fork travel and "170 mm F.O.L.D. Gen2" frame travel (cataloged at 180/160 using the FOCUS FAQ\'s explicit "170 mm at the fork and 160 mm at the rear triangle" answer, which conflicts with the SKU page\'s own 180mm fork figure — both maker-stated, discrepancy disclosed rather than silently resolved), "Bosch Performance Line CX Smart System, 85 Nm" motor, "Rock Shox Super Deluxe Coil Select, 230/65 mm" shock, "7005 hydroformed aluminium" frame, price EUR 6,999.00. Drivetrain/brakes not stated on the fetched sections, left uncataloged. verify/emtb-brands-2 (2026-07-23): re-fetched this exact product-id-92485 SKU page — its own spec table states "Bosch PowerTube Li-Ion (750Wh)" and "26.7 kg (750 Wh)" complete-bike weight, both maker-stated for THIS SKU specifically (the 800Wh figure was the general SAM² overview page\'s "up to 800Wh" upgrade-option language, not this exact SKU\'s shipped spec) — CORRECTED batteryWh 800 -> 750, added weight 26.7kg. PRICE: EUR 6,999.00 maker-published converted at the 2026-07-23 open.er-api.com EUR->USD rate (1.138703) -> $7,969.78; Focus has no consistent US retail MSRP for this SKU (the fetched pages are EU/AU market pages) — disclosed regional conversion, not a US MSRP.',
    verified: true, priceBasis: 'regional-conversion', lastChecked: '2026-07-23', source: 'https://www.focus-bikes.com/es_en/catalog/product/view/_ignore_category/1/id/92485/s/focus-sam2-6-8-2/'
  },
...

  {
    ...
    disciplines: ['e-trail'], weight: 18490, price: 13435.56, modelYear: 2026,
    ...
    desc: 'Sourced from BikeRadar\'s and MBR\'s independent reviews, both quoting Forestal\'s own spec sheet: BikeRadar ("Motor Forestal EonDrive motor, 360Wh Aurora Performance battery... up to 60Nm of torque and 250W of nominal power... built by Bafang... 150mm-travel", 18.49kg, price EUR 11,799/GBP 10,999) and MBR ("60Nm torque from the Bafang-built EonDrive motor, paired with a 360Wh battery... Frame: Alpha Box Carbon, 150mm travel... Fork: RockShox Pike Ultimate... Motor: Forestal EonDrive, 400W/60Nm"). Introduces motorBrand:"forestal" to EMTB_VOCAB (distinct from the existing Siryon row\'s "avinox" motorBrand — the Cyon\'s EonDrive is a different, Bafang-built, lightweight-class motor, not the AVINOX full-power unit). catalog/emtb-breadth-brands-1 wave (Forestal deepened from 1 row to 2). verify/emtb-brands-2 (2026-07-23): fetched forestal.com/en/products/cyon directly — its own platform spec table states motor "F60-S1... 1.95kg... 250W / 60 Nm" and battery "Aurora... 360Wh 51V", both matching this row exactly (the maker\'s page doesn\'t split out the Halo/Neon/Diode tiers by name, but confirms the shared motor/battery platform every review already attributed to Neon). PRICE: EUR 11,799.00 (BikeRadar\'s quoted EU price, the closest to a real maker RRP found) converted at the 2026-07-23 open.er-api.com EUR->USD rate (1.138703) -> $13,435.56.',
    verified: true, priceBasis: 'regional-conversion', lastChecked: '2026-07-23', source: 'https://forestal.com/en/products/cyon'
  },
...

  {
    ...
    disciplines: ['e-trail'], weight: 24400, price: 4553.68, modelYear: 2026,
...

  {
    ...
    batteryWh: 630, batteryRemovable: true, assist: 'full-power', display: 'top-tube',
    wheelConfig: '29', suspension: 'full', travelFront: 170, travelRear: 160,
    frameMaterial: 'carbon', drivetrain: 'SRAM GX Eagle 12-speed', brakes: 'SRAM DB8 4-piston (220/200mm SRAM Centerline rotors)',
    disciplines: ['e-enduro'], weight: 24600, price: 6999, modelYear: 2026,
    note: 'Spanish motorcycle brand GasGas\' entry point into the e-enduro ECC range, alongside the already-cataloged flagship ECC 6 — same SRAM Eagle Powertrain drive unit and carbon frame w/ integrated battery, stepped down to a mechanical GX Eagle drivetrain and SRAM DB8 brakes.',
    desc: 'Fetched directly from the maker\'s own product page verify/emtb-verify-1 (gasgas.com/bicycles/en-us/bikes/enduro/e-bike/ecc/ecc-4.html) via the browser pane (same JS-rendered wall as the ECC 6 sibling row, cleanly rendered): "FRAME 29\\", Carbon, Integrated battery, Boost, 160 mm", "FORK RockShox ZEB Select+, ... 170 mm, tapered", "SHOCK RockShox Super Deluxe Select+, Coil, 205x65 mm", "MOTOR SRAM Eagle Powertrain Drive Unit, 250 W, 90 Nm", "BATTERY SRAM Eagle Powertrain, 630 Wh", "FRONT/REAR BRAKE SRAM DB8, 4-piston... 220 mm / 200 mm SRAM Centerline rotors", "REAR DERAILLEUR SRAM GX Eagle, 12-S", "WEIGHT 24.6 kg", listed at "USD 6,999.00 MSRP plus USD 60 freight". Every field the prior credible-source breadth pass entered (drivetrain/brakes/travel/price) matches the maker page exactly; this fetch additionally confirms the weight (24.6kg, was uncataloged) and upgrades the price to a directly maker-fetched MSRP (freight excluded, matching this catalog\'s MSRP-only pricing convention).',
    verified: true, priceBasis: 'msrp-confirmed', lastChecked: '2026-07-23', source: 'https://www.gasgas.com/bicycles/en-us/bikes/enduro/e-bike/ecc/ecc-4.html'
  },

  {
    id: 'em-rotwild-rex900-core', cat: 'emtb', brand: 'Rotwild', model: 'R.EX 900 CORE',
    motorBrand: 'dji', motorModel: 'DJI Avinox M1.24 Drive Unit', motorTorque: 105, motorPowerPeak: 1000,
    batteryWh: 864, batteryRemovable: true, assist: 'full-power',
    wheelConfig: 'mullet', suspension: 'full', travelFront: 160, travelRear: 150,
    frameMaterial: 'carbon',
    disciplines: ['e-enduro'], weight: 22800, price: 10236.98, modelYear: 2026,
    ...
    desc: 'Unverified sample via the credible-source breadth policy — cross-confirmed across Bikerumor ("DJI Avinox M1 motor... 105 Nm torque and 1000 W peak power... time limited boost mode bumping it up to 120 Nm... 864 Wh battery") and EMTB Forums\' bike-finder listing plus E-MOUNTAINBIKE Magazine\'s R.EX news coverage (mullet wheel concept, three trims "CORE at EUR 8,990, PRO at EUR 9,990, ULTRA at EUR 12,490"). catalog/emtb-breadth-3 wave (Rotwild deepened from 1 row to 2). verify/emtb-brands-2 (2026-07-23): fetched rotwild.com/en/r-ex-core directly — its own spec table states "Motor: DJI M1.24 / 105 Nm... Battery: IPU900 / 864 Wh... Front/rear suspension travel: 160 mm / 150 mm... Bike weight: 22.8 kg", price "8.990,00 €". CORRECTED travel 170/160 -> the maker-stated 160/150mm (a real prior-wave error, not a generational drift); added maker-stated weight 22.8kg (superseding the previously-uncataloged "22.4kg system weight only" caveat, since the maker page states 22.8kg as the complete "Bike weight" figure). PRICE: EUR 8,990 maker-confirmed converted at the 2026-07-23 open.er-api.com EUR->USD rate (1.138703) -> $10,236.98.',
    verified: true, priceBasis: 'regional-conversion', lastChecked: '2026-07-23', source: 'https://www.rotwild.com/en/r-ex-core'
  },
...

  {
    ...
    disciplines: ['e-trail', 'e-enduro'], weight: 24300, price: 7959.50, modelYear: 2026,
    note: 'Mid-tier trim of Thok\'s TP4 all-mountain/enduro platform, alongside the already-cataloged base TP4 — same carbon frame/motor/battery, upgraded to FOX Performance suspension (36 fork) and DT Swiss H1900 wheels; the TP4-S/TP4-R share identical builds (only frame-manufacturing origin differs, Taiwan vs Italy). Modular battery: this row is the 800Wh (heaviest, longest-range) configuration; 600Wh/400Wh options are also sold.',
    desc: 'Unverified sample via the credible-source breadth policy — velomotion.net\'s dedicated "Thok TP4-S & TP4" launch coverage and ridewill.com/mtbdatabase.com\'s TP4-R spec listing (identical build to the S) confirm "FOX Performance suspension with a 36-inch fork combined with DT Swiss H1900 wheels, a Shimano SLX-XT drivetrain, and Magura MT5 four-piston brakes... The TP4-S costs EUR 6,990, while the TP4 costs EUR 5,990". Frame/motor/battery/travel carried forward from the already-cataloged base TP4 row (same platform). verify/emtb-brands-2 (2026-07-23): fetched thokbikes.com\'s own TP4-S product pages directly (thokbikes.com/en/negozio/ebike-en/tp4-s-black/ + the Dragon Green/Forge Red colorway variants) — confirms "UD carbon frame (travel: 160mm front / 150mm rear), Bosch Performance Line CX motor, 800 Wh modular battery... FOX Float 36 Performance fork, FOX Float X shock, Magura MT5 brakes, 29\\" wheels", "800Wh battery -> 24,3 kg" (added maker-stated weight for the 800Wh build cataloged here), price EUR 6,990 (matches existing estimate exactly). Motor torque is stated INCONSISTENTLY across the maker\'s own colorway pages (Black/Royal Black variant states "120 Nm max torque", the Dragon Green variant states "100 Nm" for what reads as the identical Performance Line CX BDU384Y unit) — flagged as a disclosed maker-page discrepancy rather than silently picking one; this row keeps the catalog\'s existing 120Nm figure (the majority-page value) rather than asserting a resolved number. PRICE: EUR 6,990 maker-confirmed converted at the 2026-07-23 open.er-api.com EUR->USD rate (1.138703) -> $7,959.50.',
    verified: true, priceBasis: 'regional-conversion', lastChecked: '2026-07-23', source: 'https://www.thokbikes.com/en/negozio/ebike-en/tp4-s-black/'
  },
...

  {
    ...
    disciplines: ['e-trail'], weight: 25000, price: 5704.25, modelYear: 2026,
    note: 'Austrian maker Simplon\'s new-for-2026 aluminum trail e-MTB, alongside the already-cataloged carbon Rapcon :e — one of three new AL:e aluminum models (Rift/Stomp/Rapcon AL:e) launched April 2026 as lower-priced alternatives to Simplon\'s carbon range.',
    desc: 'Fetched directly from the maker\'s own product page (simplon.com/en/Bikes/E-Mountain-Bikes/Rift-AL-e_b_1149834 — the earlier "redirected/incomplete" fetch was a WebFetch limitation; an Exa fetch verify/emtb-verify-1 returned the full Facts & Figures table): "Frame material Aluminium", "Travel (rear/front) 140 mm / 140 mm", "Motor Bosch PL PX / CX Gen5", "Battery 600 / 800 Wh", "Total bike weight from 25,0 kg", "fork travel/length/OFFSET 140/556/44 mm", "shock size (eye to eye length x stroke) 185 x 55 mm", "lower shock hardware Trunnion", starting price "from EUR 4,999.00". Cataloged at the smaller 600Wh Core-tier battery/price (the page\'s own cheapest "from" figure, matching this row\'s Core trim name); brakes/drivetrain not stated on the fetched page, left uncataloged. PRICE: EUR 4,999.00 ("from") maker-published starting price converted at the 2026-07-23 open.er-api.com EUR->USD rate (1.141079) -> $5,704.25, replacing the prior session\'s derived estimate now that the real maker figure is fetched; Simplon has no US retail presence, no USD MSRP exists — disclosed regional-conversion "from" sample. catalog/emtb-breadth-3 wave (Simplon deepened from 1 row to 2); promoted to verified:true and weight/price corrected verify/emtb-verify-1.',
    verified: true, priceBasis: 'regional-conversion', lastChecked: '2026-07-23', source: 'https://www.simplon.com/en/Bikes/E-Mountain-Bikes/Rift-AL-e_b_1149834'
  },
...

  {
    ...
    desc: 'Unverified sample via the credible-source breadth policy — The Loam Wolf\'s 2026 eMTB Shootout review and intensecycles.com\'s own blog repost of that review, plus BikeMag\'s and Pinkbike\'s first-look coverage, consistently confirm "Bosch CX Gen 5 drive unit... 120Nm peak torque and 750W peak power... 600Wh or 800Wh battery options... carbon fiber frame... Dual Co-Rotating (DCR) link suspension... 150mm rear travel, with 160mm front travel... Foundation at $6,999 or the Pro at $8,999... medium Pro build weighs 50lbs 6oz (22.85kg)". Cataloged at the larger 800Wh option (matching the quoted weight figure\'s own build) and the Pro-trim price. catalog/emtb-breadth-3 wave (Intense deepened from 2 rows to 3 — this row and the existing Tazer MX pair now span two full platform generations). verify/emtb-brands-2 (2026-07-23): fetched intensecycles.com/products/tazer-pro directly — its own build-spec table states "Motor: Bosch CX Gen 5, 120Nm", "Battery: Bosch 800Wh PowerTube", price "$8,999.00", and full drivetrain (GX Eagle Transmission RD-GX-E-B1, XS-1275 cassette), fork (RockShox ZEB Ultimate 160mm 29), shock (RockShox Super Deluxe Ultimate 210x55), brakes (SRAM Maven Silver, HS2 200mm) — every field this row carries matches exactly. Frame-only weight is published (24lb 5oz/11.03kg, size L) but that is not a complete-bike figure, so the existing 22.85kg complete-bike sample (The Loam Wolf\'s tested unit) stays as-is.',
    verified: true, priceBasis: 'msrp-confirmed', lastChecked: '2026-07-23', source: 'https://intensecycles.com/products/tazer-pro'
  },
...

  {
    ...
    wheelConfig: '29', suspension: 'full', travelFront: 170, travelRear: 170,
    frameMaterial: 'carbon', drivetrain: 'SRAM Eagle 90 T-Type Transmission (XS-1270 Eagle T-Type cassette)', brakes: 'SRAM Maven BSE SJ (220mm)',
    disciplines: ['e-enduro'], weight: null, price: 8311.40, modelYear: 2026,
    note: 'French maker Lapierre\'s new-for-2026 all-mountain/enduro carbon flagship, alongside the already-cataloged Overvolt AM 10.8 and Overvolt TR 7.8 rows — full-carbon frame (Mitsubishi 24T/40T layup mix, 2.9kg size-M frame-only), full 29in wheel platform, entry build of a 3-model range (6.8/7.8/10.8).',
    desc: 'Unverified sample via the credible-source breadth policy — off-road.cc\'s and BikeBiz\'s launch coverage of this exact platform, plus alltricks.com\'s live SKU listings for the 6.8/10.8 builds, confirm "Bosch Performance CX motor with 100Nm of torque... 800Wh removable battery... compatible with Bosch\'s PowerMore range extender... 170/165mm of travel... carbon frame weighing 2.9kg (size medium)... mullet configuration and features a two-position flip chip... Starting price: EUR 7,299" (the 6.8, entry build). alltricks.com\'s own 10.8 listing (a higher trim) independently confirms the shared "Bosch 100 Nm MX motor" designation and 800Wh battery. No complete-bike weight found for the 6.8 build specifically. verify/emtb-brands-2 (2026-07-23): fetched lapierrebikes.com\'s own current SKU page directly (lapierrebikes.com/en-int/products/overvolt-am-cf-68-lorua, SKU LORUA390) — confirms "Battery: Bosch Powertube 800 Wh", "Motor: Bosch Performance line CX", price EUR 7,299 (all matching), but CORRECTS wheelConfig mullet -> 29 (the maker\'s own configurator shows "Wheelsize: 29" for this SKU, not a mullet option) and travelRear 165 -> 170mm ("Rockshox Deluxe Select+ 205x60mm Trunnion... Travel: 170 mm"); adds the real drivetrain (SRAM Eagle 90 T-Type Transmission w/ XS-1270 Eagle T-Type cassette) and brakes (SRAM Maven BSE SJ, 220mm), both previously uncataloged. Motor torque (100Nm, "MX tune") is not itemized on the maker\'s own page (no Nm figure shown) — kept from the prior press-sourced figure, not independently re-confirmed this pass. No complete-bike weight found. PRICE: EUR 7,299 maker-confirmed converted at the 2026-07-23 open.er-api.com EUR->USD rate (1.138703) -> $8,311.40.',
    verified: true, priceBasis: 'regional-conversion', lastChecked: '2026-07-23', source: 'https://lapierrebikes.com/en-int/products/overvolt-am-cf-68-lorua'
  },
...

  {
    ...
    desc: 'Sourced from Privateer\'s own product page (privateerbikes.com/products/privateer-e161, cached snapshot — live fetch was rate-limited at research time) cross-checked against Vital MTB\'s detailed spec table (most complete third-party source). Rear travel is disclosed as 161mm on Privateer\'s own sheet and Vital MTB\'s table (one Cycling Electric mention says 160mm; 161mm treated as authoritative, matching the "161" model name). Weight 25.5kg (56lb 3.5oz), size not specified in any source. PRICE: Privateer\'s GBP price has moved substantially over time (£5,999 at 2023 launch -> £6,499 later -> a reported cut to £3,999.99 per Cycling Electric, June 2024) — flagged as a dated, unconfirmed-current price, not a live MSRP. catalog/emtb-depth-5 wave (Privateer absent -> 1 row, single-spec product). verify/emtb-brands-2 (2026-07-23): re-fetched privateerbikes.com/products/privateer-e161 directly (resolves cleanly now, past the prior rate-limit) — re-confirms "Shimano EP801, 85NM" motor and "Shimano BT-EN806, 630Wh" battery exactly matching this row; Vital MTB independently corroborates the same motor/battery plus a UK price of GBP 6,499 (a different, higher figure than the GBP 3,999.99 sale price this row\'s price derives from — the current live product page itself shows no price to WebFetch\'s render, so neither GBP figure is re-confirmed as CURRENT this pass). Interfaces meet the verification bar; price stays the existing disclosed dated estimate, no priceBasis token.',
    verified: true, lastChecked: '2026-07-23', source: 'https://www.privateerbikes.com/products/privateer-e161'
  },
  ...
  },
  {
    ...
    desc: 'Motor/battery/wheel confirmed via flyer-bikes.com\'s own Uproc EVO:X page; travel figures from ebike-mtb.com\'s current-gen coverage (180/170mm, vs an earlier 170/165mm gen — flagged as a generational spec, not fabricated). Drivetrain/brakes/weight/price for the 8.70 build from an ebike24.com retailer listing (fork/shock/drivetrain/brakes/weight/price all matched one consistent trim). verify/emtb-brands-2 (2026-07-23): re-fetched flyer-bikes.com/gb-en/uprocevox directly — its own spec table states "800 Wh / Battery capacity", "180 / 170 mm / Travel", "Performance Line CX (85 Nm) / Bosch Smart System", "29\'\' / 27.5\'\' / Mixed Wheel" for the PLATFORM as a whole; this row\'s trim-specific 750Wh figure (a smaller pack than the platform\'s 800Wh top spec) is independently confirmed by two trim-specific third-party sources (emtb-test.com\'s "8999 €... Battery 750" and digitec.ch\'s listing, both naming the "8.70" trim explicitly) — the family page states the platform maximum, not every trim\'s battery. Travel (180/170) and torque (85Nm) now maker-page-confirmed directly; price EUR 8,999 independently matches emtb-test.com\'s trim-specific listing exactly.',
    verified: true, priceBasis: 'third-party-listed',
  ...
  // --- catalog/emtb-depth-10: third-model depth on thin brands (BH, Simplon) ---
  {
    ...
    desc: 'Sourced from BIKE Magazin\'s fetched hands-on feature (bike-magazin.de/en/enduro-bikes/bh-bikes-ilynx-sl-lots-of-power-low-weight-spanish-e-mtb-for-trail-and-enduro/), quoting: "Motor: Shimano EP801", "Battery: 630 Wh, permanently installed", "Optional: additional battery with 180 Wh", "3 Enduro and 4 Trail variants from 6999 euros", "Suspension travel: 160 mm (Enduro) or 140 mm (Trail)", "Wheel size: 29 inch", "Weight: from 18.9 kilos (top model Trail 0.0, manufacturer\'s specification)", "Carbon frame". PRICE: EUR 6,999 ("from", lineup-starting price across all Trail/Enduro variants) converted at the 2026-07-23 open.er-api.com EUR->USD rate (1.138703) -> $7,969.77 — an editorial-disclosed starting price, not a maker-fetched fixed-SKU MSRP. verify/emtb-brands-2 (2026-07-23): re-fetched bhbikes.com\'s own iLynx+ SL platform pages directly (bhbikes.com/en_EE/e-mtb/trail/ilynx_plus_sl + bhbikes.com/en_GB/bh-unique/ilynx-sl-trail-8-5) — confirms "EP801 | 85 NM" and "All models come standard with a 630Wh battery... The additional XPro DD external battery is optionally available and provides an extra 180 Wh" (810Wh combined), matching motorTorque/batteryWh/rangeExtender exactly. Drivetrain/brakes still vary by numbered trim (8.5/8.7 Carbon/etc.) and aren\'t itemized for the generic "Trail" name this row uses, so stay uncataloged.',
    verified: true, priceBasis: 'regional-conversion', lastChecked: '2026-07-23', source: 'https://www.bhbikes.com/en_EE/e-mtb/trail/ilynx_plus_sl'
  },
  {
