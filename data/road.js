...
var ROAD_PARTS = [
  ...
  { id: 'sh-sram-red-axs', cat: 'shifter', brand: 'SRAM', model: 'RED AXS Shifter/Brake Lever (pair)', family: 'sram-red-axs',
    ...
    note: 'road-8 wave (2026-07-20): browser-pane re-fetch of sram.com/en/sram/models/ed-red-e1 confirms road-7\'s flag - this is SRAM\'s ONLY published price/SKU covering RED AXS HRD levers+calipers: "RED AXS HRD Shift-Brake System" (ED-RED-E1), $745 MSRP, 1378g, "LEVER ORIENTATION Left, Right" / "BRAKE ORIENTATION Front, Rear" / "DEFAULT FUNCTION AXS - Front Derailleur, AXS - Rear Derailleur" (confirms frontShift:true, axs-wireless, hydraulic disc). SRAM sells no separate lever-only or caliper-only RED AXS SKU/price at sram.com - this catalog\'s schema splits the one real $745 bundle across two engine slots (this shifter row + br-sram-red-axs) purely because checkBuild models front/rear brakes as their own slot, the same shape as every other hydraulic-lever brand in this file. RESOLVED the $1250+$400=$1650 conflict (well above the real $745 bundle) by re-splitting proportionally to the prior (unsourced) 1250:400 ratio so the two rows sum EXACTLY to the one number SRAM actually publishes: 565 + 180 = 745. Both figures remain unverified per-slot ESTIMATES (no maker source states a per-slot price - flagging that split as sourced would overclaim), but the total is now the real $745 MSRP rather than an invented $1650. Weight (370g) is unchanged/still unsourced - the 1378g bundle figure spans lever pair + caliper pair + hydraulic fluid + hardware, not cleanly divisible to this row alone.' },
  ...
  { id: 'br-sram-red-axs', cat: 'brake', brand: 'SRAM', model: 'RED AXS HRD Caliper (pair)', family: 'sram-red-axs-brake',
    ...
    note: 'road-8 wave (2026-07-20): price re-split alongside its leverPair sh-sram-red-axs to resolve the road-7-flagged $1250+$400=$1650-vs-$745 conflict - see that row\'s note for the sram.com/en/sram/models/ed-red-e1 fetch (SRAM sells RED AXS levers+calipers as one $745 "ED-RED-E1" bundle SKU, no separate caliper-only price published). 180 + sh-sram-red-axs\'s 565 = 745, the real bundle MSRP, in place of the prior unsourced $400. Still an unverified per-slot estimate (no maker source states a caliper-only price).' },
