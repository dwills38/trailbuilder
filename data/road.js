...
var ROAD_PARTS = [
  ...
  { id: 'fk-specialized-tarmac-sl8', cat: 'fork', brand: 'Specialized', model: 'Tarmac SL8 Fork', family: 'specialized-tarmac-sl8',
    ...
    verified: true, lastChecked: '2026-07-21', source: 'https://www.specialized.com/us/en/s-works-tarmac-sl8-frameset/p/216956',
    note: 'road-14 wave: specialized.com Tarmac SL8 frameset page confirms fork 12x100mm thru-axle, flat-mount disc, 32mm tire clearance (matches SL7); tapered steerer per Specialized S-Works replacement-fork listings. No per-config fork weight published — 370g kept as a nominal/sample figure (forks interface-verification exception).' },
  ...
  { id: 'fw-campagnolo-zonda-c17-disc', cat: 'frontwheel', brand: 'Campagnolo', model: 'Zonda C17 Disc', family: 'campagnolo-zonda-c17',
    ...
    verified: true, lastChecked: '2026-07-21', source: 'https://web.archive.org/web/20241005194233/https://www.campagnolo.com/us-en/zonda-disc-brake/WWRZONDADB.html',
    note: 'road-11 wave (wheels/cockpit tail): campagnolo.com\'s LIVE site 404s the old WWRZONDADB.html SKU page — the "Zonda" name has been reused for an all-new full-carbon wheel in Campagnolo\'s current 2026 catalog (confirmed: campagnolo.com/us-en/zonda/WWRZONDACARBON.html lists "Zonda ALL ROAD - FULL CARBON - HIGH PROFILE - DISC", a different product entirely). Resolved via a Wayback Machine snapshot of the old SKU page (2024-10-05, pre-rebrand, this project\'s standard archive-step fallback): confirms disc brake, "AFS disc interface" + "6 BOLTS AND AFS" (two rotor-mount SKUs sold; this row models the AFS/Center-Lock-style one, matching this file\'s established AFS-is-Campagnolo\'s-Center-Lock-name convention from the gravel-6 rotor pass), "Compatible with 12 mm pivot, 100/142 mm flange" (confirms hub:\'12x100\'), "Weight: 1675 g" — an EXACT match to this row\'s pre-existing ~1675g pair-total estimate (now maker-confirmed, not just retailer-corroborated). INTWIDTH CORRECTED 17->22: the archived page states "Profile width: 22 mm" nowhere calling out "C17"/17mm at all — Campagnolo\'s own site dropped that literal figure/name even before the 2026 carbon rebrand, though retailers (Halfords, Condor Cycles, Bike-Discount) still market this exact wheel as "Zonda C17" today; model string kept as the retail-recognized name, but the sourced 22mm figure wins over the unsourced 17mm per THE BAR (a real number beats a guess even when it makes the model name read oddly — flagged for the coordinator). No maxTire figure is stated on the archived page ("Wheel for clincher tyre" only) — stays the pre-existing sample. Price: the archived page states "Price from € 659" (a whole-pair EUR figure, ~$700-750 at typical FX) — not converted into this row\'s USD price field per this project\'s no-live-FX convention; the existing $550 stays an unsourced sample, now flagged as likely low vs. the sourced EUR figure.' },
  { id: 'rw-campagnolo-zonda-c17-disc', cat: 'rearwheel', brand: 'Campagnolo', model: 'Zonda C17 Disc', family: 'campagnolo-zonda-c17',
    ...
    verified: true, lastChecked: '2026-07-21', source: 'https://web.archive.org/web/20241005194233/https://www.campagnolo.com/us-en/zonda-disc-brake/WWRZONDADB.html',
    note: 'road-11 wave (wheels/cockpit tail): see front-wheel note (fw-campagnolo-zonda-c17-disc) for the full sourcing/supersession finding — same archived Campagnolo page, same 1675g pair total (this row\'s 921g stays the estimated ~55% share, still not a page-stated per-wheel split) and same intWidth correction (17->22). The archived page does not itemize a driver-body/freehub spec field (Campagnolo product pages commonly omit this, matching the gravel-6 wave\'s finding on Campagnolo rotor pages) — freehub:\'campag-11\' stays the reasonable assumption for the Campagnolo-driver SKU (a Shimano-HG-driver version of this same wheel is also sold, per the road-5 gap-fill\'s original note), not independently confirmed this pass.' },
  ...
  { id: 'sp-canyon-s27-vcls-aeroad', cat: 'seatpost', brand: 'Canyon', model: 'S27 VCLS CF Seatpost', family: 'canyon-s27-vcls',
    ...
    verified: true, lastChecked: '2026-07-21', source: 'https://www.canyon.com/en-us/gear/components/spare-and-wear-parts/spare-wear-parts/canyon-s27-vcls-cf-seatpost/101402.html',
  ...
  { id: 'sp-trek-emonda-slr-mastcap', cat: 'seatpost', brand: 'Trek', model: 'Émonda SLR Carbon Mast Cap', family: 'trek-emonda-slr-mastcap',
    ...
    weight: 92, price: 179.99,
    note: 'oem-posts-1 new row (2026-07-21): the Émonda SLR frame\'s own OEM seatpost — added so fr-trek-emonda-slr\'s seatpost:\'proprietary\' has a real matching part. This is a proprietary Trek/Bontrager round CARBON SEATMAST design (WebSearch-corroborated, trekbikes.com FAQ + multiple Trek-authorized-dealer listings, e.g. tritoncycles.co.uk): the frame\'s own mast tube is not cut to length like a standard round seatpost — only a mast CAP is a separate purchasable part, "compatible with all Trek road bikes with round seat masts" and "the same mast cap found on the Émonda SLR 10". 20mm offset, 135mm/175mm cap lengths, "50g lighter than the standard Trek round seat cap" at approx 92g (dealer-stated). Price $179.99 is the US regular (non-sale) MSRP per an authorized Trek dealer (Wheel & Sprocket) selling the identical cap at $79.99 explicitly "on sale from $179.99 regular price" — the discounted figure was not used per the MSRP-only pricing policy. Not marked verified:true: weight/price are dealer-stated, not a direct trekbikes.com spec-table fetch (the product page did not render a spec table this session).' },
  { id: 'sp-trek-domane-slr-kvf', cat: 'seatpost', brand: 'Trek', model: 'KVF Aero Carbon Seatpost', family: 'trek-domane-slr-kvf',
    ...
    weight: 103, price: 263.99,
    note: 'oem-posts-1 new row (2026-07-21): the Domane SLR frame\'s own OEM seatpost — added so fr-trek-domane-slr\'s seatpost:\'proprietary\' has a real matching part. Confirmed via trekbikes.com\'s own Domane SL/SLR/RSL FAQ: "No, it\'s a D-shaped KVF post for improved aerodynamics" — NOT a standard round tube, genuinely proprietary to fit the frame\'s IsoSpeed decoupler. Real part numbers W5285075/76/77/78 (280mm and 320mm lengths, 5mm and 20mm offsets); this row uses the 280mm/5mm point (W5285077). Weight 103g per a third-party measured/retailer spec (bike24.com); price $263.99 read off trekbikes.com\'s own US product-page JSON-LD for this SKU family (a WebSearch snippet — the page itself did not render the spec table live this session, and the JSON-LD\'s product name field read "Trek Madone Micro-adjust Aero Carbon Seatpost" rather than the Domane-branded name shown elsewhere on the same page, an inconsistency in Trek\'s own site data — flagged; the part number/URL match this Domane KVF post, not a Madone part, so used here regardless). Not marked verified:true given that flagged inconsistency.' },
  { id: 'sp-trek-madone-gen8', cat: 'seatpost', brand: 'Trek', model: 'Madone Gen 8 Seatpost', family: 'trek-madone-gen8',
    ...
    verified: true, lastChecked: '2026-07-21', source: 'https://www.trekbikes.com/us/en_US/equipment/cycling-components/bike-seatposts-accessories/bike-seatposts/trek-madone-gen-8-seatpost/p/W5316714/',
  ...
  { id: 'sp-cannondale-synapse-save', cat: 'seatpost', brand: 'Cannondale', model: 'SAVE Carbon Road Seatpost 27.2', family: 'cannondale-save-seatpost',
    proprietary: true, forFrames: ['fr-cannondale-synapse'], setback: 0,
    weight: 202, price: 200,
    note: 'oem-posts-1 new row (2026-07-21): the Synapse frame\'s own OEM SAVE-profile post — added so fr-cannondale-synapse\'s seatpost:\'proprietary\' has a real matching part (Cannondale\'s "SAVE" carbon layup is a tuned-flex proprietary post family shipped on Synapse/SuperSix frames, sold aftermarket as a genuine 27.2mm-diameter SKU rather than a generic 27.2 round post — kept proprietary/forFrames per this file\'s existing BMC-URS/Cinelli precedent rather than reclassified to plain 27.2, since the Synapse\'s own seat-tube/clamp geometry is designed specifically around this post\'s SAVE profile). Weight 202g (27.2x400mm point) and price $200 (0mm-offset SKU) per CannondaleExperts.com, a US Cannondale-authorized parts retailer — not an independent cannondale.com spec-table fetch (the page did not render pricing in this session\'s fetch), so not marked verified:true.' },
  { id: 'sp-giant-tcr-variant-2024', cat: 'seatpost', brand: 'Giant', model: 'Variant Seatpost for 2024+ TCR', family: 'giant-tcr-variant',
    ...
    note: 'oem-posts-1 new row (2026-07-21): the TCR Advanced SL frame\'s own OEM aero seatpost — added so fr-giant-tcr-advsl\'s seatpost:\'proprietary\' has a real matching part. DIRECTLY FETCHED giant-bicycles.com/gb\'s "Variant Seatpost for 2024+ TCR" product page (WebSearch-surfaced, giant-bicycles.com own copy): "Optimized for maximum aero performance at a super low weight", "Compatible with model year 2024 and on TCR frames... Not compatible with pre-model year 2024 TCR", Diameter "Aerosystem Shaping" (non-round, genuinely proprietary), Weight "195g", full carbon. Price £149.99 (UK giant-bicycles.com listing) — flagged as a converted-basis USD estimate rather than a true US MSRP; a US retailer listing for the same part independently showed a comparable $146.99, corroborating the conversion is in the right range.' },
