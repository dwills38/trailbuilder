...
/** @typedef {'exo'|'exo-plus'|'doubledown'|'dh'|'trail'|'enduro'|'downhill'|'super-race'|'super-ground'|'super-trail'|'super-gravity'|'super-downhill'|'protection'|'hardwall'|'prowall'|'tcs-light-sg2'|'tcs-tough-sg1'|'tcs-tough'|'atc'|'aec'|'agc'|'xc-race'|'gravity-shield'|'grid-trail'|'grid-gravity'|'grid'|'control'|'trc'|'grc'|'xcc'|'enduro-core'|'synthesis'|'gravity-core'|'gxe-core'|'dh-core'|'wire-bead'|'folding-tlr'|'inner-strength'|'core-strength'|'pro-xr'|'downhill-strength'|'high-output'|'super-tough'|'durable'|'ultra-durable'|'light-and-supple'|'light-trail'|'tough'|'tr'|'emc'|'eps'|'eps-tlr'|'exo-btb'|'flux-gr'|'flux-am'|'flux-gr-radial'|'hardskin'|'sideskin'|'gravity-pro'|'trail-pro'|'ksct'|'xc-pro'|'bikepark'} Casing */
 ...
 * Mirrors schema.js's VOCAB.priceBasis - keep the two in step. @typedef {'msrp-confirmed'|'discontinued-no-msrp'|'oe-only-no-msrp'|'regional-conversion'|'bundle-split-estimate'|'third-party-listed'|'pair-split-estimate'|'deprioritized-no-price-source'} PriceBasis */
 ...
 * @property {string} [position] the physical POSITION on the bike this slot
 *   fills, shared with its peers (today only 'seat': dropper + rigid seatpost).
 *   Unlike `altOf` (an AND path — frontWheel needs a hub AND a rim), position
 *   peers are an OR: EITHER slot fills the position, so completeness counts the
 *   position once — see positionPeersOf()/wheelPositionFilled() in compat.js.
 */
