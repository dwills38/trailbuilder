...
var PARTS = [
  ...
  /* --- catalog-frames-holdouts (2026-07-08): the Specialized Epic 8 family. Prior passes SKIPPED these because no fetched page STATED the rear brake mount. Resolved: every Epic 8 trim ships a SRAM Level/Code caliper, which is POST MOUNT (SRAM makes no flat-mount MTB caliper), so the frame is PM - a sound interface pin, documented per-row. Its 190x45 std shock already exists in-catalog (sh-rockshox-sidluxe-ultimate-190x45). Scott Spark/Genius were re-evaluated and SKIPPED: proprietary integrated Nude T165x45 + TwinLoc, no maker aftermarket-shock policy (fit minefield). --- */
  { id:'fr-specialized-epic-8', cat:'frame', brand:'Specialized', model:'Epic 8', family:'specialized-epic', gen:'8', modelYear:2024, disciplines:['xc'], price:3000, weight:1900, wheelConfigs:['29'], rearAxle:'Boost148', headset:'tapered', bb:'BSA73', seatTube:30.9, brakeMount:'PM', maxRotorR:180, suspension:'full', shockEye:190, shockStroke:45, shockMount:'std', travel:120, maxForkTravel:130, udh:true, bundledShock:null, frameOnly:true, desc:'price/weight/maxRotorR/maxForkTravel = sample (specialized.com deep pages fetch-blocked; S-Works frameset ~$3k, ~1.9kg claimed); 120mm rear + RockShox SIDLuxe Ultimate 190x45 STANDARD mount from the FETCHED Specialized Epic 8/EVO 2024 owner manual ("...Solo Air, 190x45mm") + FETCHED Vital EVO spec table (FOX FLOAT 190x45); 12x148 Boost + SRAM DUB BSA73 threaded + 30.9 post + UDH from that manual/Vital; brakeMount PM INFERRED from the stock SRAM Level Ultimate/Code post-mount caliper (SRAM makes no flat-mount MTB caliper) - no fetched source states the mount word; maxRotorR 180 = EVO ships 180 rear stock + forum-demonstrated on the base (base ships 160); maxForkTravel 130 per Flow ("Epic 8...120-130mm forks"; design 120); maxTire omitted - base race clears ~2.35, EVO 2.4, left dormant vs a false green' },
  { id:'fr-specialized-epic-hardtail', cat:'frame', brand:'Specialized', model:'Epic Hardtail', family:'specialized-epic-ht', modelYear:2024, disciplines:['xc'], price:2200, weight:900, wheelConfigs:['29'], rearAxle:'Boost148', headset:'tapered', bb:'BSA73', seatTube:30.9, brakeMount:'PM', maxRotorR:180, suspension:'hardtail', maxForkTravel:120, udh:true, frameOnly:false, desc:'price/weight/maxRotorR/maxForkTravel = sample (design fork 100mm; complete ~$2.7k; carbon HT frame ~0.9kg); rear brake = SRAM Level TL 2-piston + 160mm rotor per FETCHED BikeRadar Epic Hardtail Comp review -> brakeMount PM (Level is post-mount; SRAM makes no flat-mount MTB caliper; the mount word is stated nowhere, this pins it); FACT 11m, 12x148 Boost, SRAM DUB BSA73 threaded, 30.9 post, UDH per FETCHED BikeRadar specs + geometrygeeks/99spokes; maxTire omitted (only the stock 29x2.35 is published, not a max); this CLEARS the prior Epic HT skip (rear-mount holdout) via the stock-caliper pin' },
